...
const title = "All Dragons — Siri Dragons Sketchfab Archive";
const description =
  "Browse the complete bestiary of Siri Dragons: every dragon, wyvern and fantasy creature published on Sketchfab, searchable and ready to view in 3D.";
...
export const Route = createFileRoute("/dragons")({
  head: () => ({
    ...
    links: [{ rel: "canonical", href: "/dragons" }],
  }),
      ...
      <main className="relative z-10 pt-32 pb-24 px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div>
              <p className="text-xs tracking-[0.3em] uppercase text-primary mb-3">— Complete Bestiary</p>
              <h1 className="font-display text-4xl md:text-6xl">All Dragons</h1>
              <p className="text-muted-foreground mt-4 max-w-2xl">
                Every dragon, wyvern and creature in the Siri Dragons archive — synced
                live from Sketchfab. Search by name or description.
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-ember bg-card/40 p-12 text-center">
              ...
              <h2 className="font-display text-2xl mb-2">
                {query ? "No dragons match that search" : "The forge is warming up"}
              </h2>
              <p className="text-muted-foreground max-w-md mx-auto">
                {query
                  ? "Try a different name or description keyword."
                  : "New dragons appear here automatically as they are published on Sketchfab."}
              </p>
            </div>
          ) : (
            <>
              <p className="text-sm text-muted-foreground mb-6">
                {filtered.length} dragon{filtered.length === 1 ? "" : "s"} · page {currentPage} of {totalPages}
              </p>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {visible.map((m) => (
